...
SaveNewStaticAdress() {
    FileDelete, ConfigLEVELING.txt
    ; Utiliser un caractère spécial (par exemple, "`n") pour séparer les valeurs sur la même ligne
    FileAppend,%duree1%`n%dureeCouleur1%`n%duree2%`n%dureeCouleur2%`n%duree3%`n%dureeCouleur3%`n%potion1%`n%couleur1%`n%potion2%`n%couleur2%`n%potion3%`n%couleur3%, ConfigLEVELING.txt
}

LoadOptions() {
    FilePath := "ConfigLEVELING.txt"
    if (FileExist(FilePath)) {
        ...
        FileReadLine, potion3, %FilePath%, 11
        FileReadLine, couleur3, %FilePath%, 12
